  ...
  >`
    select a.id, a.name, a."archivedAt",
      coalesce((select sum(c."amountCents") from "Charge" c
                where c."accountId" = a.id and c."voidedAt" is null and c."chargedOn" <= ${asOf}::date), 0)::bigint as charged,
      coalesce((select sum(p."amountCents") from "Payment" p
                where p."accountId" = a.id and p."voidedAt" is null and p."paidOn" <= ${asOf}::date), 0)::bigint as paid,
