...
import { Badge, Button, Card, Empty } from "@/src/components/ui";
...
/** One assignment. Opens read-only; Edit shows the form and the file picker. The link and the hand-ins sit under it. */
export default async function AssignmentPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ returnTo?: string; edit?: string }> }) {
  ...
  /** The attached files; with `removable`, each has a Remove link (edit mode only). */
  const files = (removable: boolean) => a.files.length === 0 ? <p className="text-sm text-muted">None attached.</p> : (
    <ul className="space-y-1 text-sm" data-testid="assignment-files">
      {a.files.map((f) => (
        <li key={f.file.id} className="flex flex-wrap items-center gap-2">
          ...
          {removable && <form action={detachFileAction}><input type="hidden" name="assignmentId" value={a.id} /><input type="hidden" name="fileId" value={f.file.id} /><Button variant="link" className="text-xs text-muted">Remove</Button></form>}
        </li>
      ))}
    </ul>
  );
  ...
  const overview = (
    <Card>
      <div className="grid gap-x-6 gap-y-4 sm:grid-cols-2 lg:grid-cols-4">
        <Fact label="Student"><Link href={`/students/${a.student.id}`} className="text-brand hover:underline">{a.student.firstName} {a.student.lastName}</Link></Fact>
        <Fact label="Due">{a.dueOn ? formatDate(a.dueOn) : <span className="text-muted">No due date</span>}</Fact>
        <Fact label="Assigned">{formatDay(a.createdAt, s.timezone)}</Fact>
        <Fact label="From the lesson">{a.lesson ? <Link href={`/lessons/${a.lesson.id}`} className="text-brand hover:underline">{formatDay(a.lesson.startsAt, s.timezone)}</Link> : <span className="text-muted">None</span>}</Fact>
        <Fact label="Instructions" className="sm:col-span-2 lg:col-span-4"><p className="whitespace-pre-line">{a.description || <span className="text-muted">None</span>}</p></Fact>
        <Fact label={`Files${a.files.length ? ` · ${a.files.length}` : ""}`} className="sm:col-span-2 lg:col-span-4">{files(false)}</Fact>
      </div>
    </Card>
  );
      ...
      actions={canEdit && (
        <>
          ...
          {a.submissions.length === 0 && (
            <ConfirmForm action={deleteAssignmentAction} message="Delete this assignment? It has no submissions, so nothing else is lost."><input type="hidden" name="assignmentId" value={a.id} /><input type="hidden" name="returnTo" value={back} /><Button variant="danger">Delete</Button></ConfirmForm>
          )}
        </>
      )}
      overview={overview}
