...
On localhost, set `DEV_AUTO_LOGIN=true` in `.env` (or an email) and pressing "Log in" opens a session for that person without a password. The landing page, the redirects, and sign out behave as in production. It does nothing outside development, and the browser tests turn it off.
